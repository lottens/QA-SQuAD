...
class Trainer(object):
    ...
    def __init__(self, args, model, loss,
                 ...
                 train_eval_file, dev_eval_file, dev_file,
                 optimizer, scheduler, epochs, with_cuda,
                 save_dir, tbx, verbosity=2, save_freq=1, print_freq=10,
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            if epoch % self.save_freq == 0:
                self._save_checkpoint(
                    epoch, result["f1"], result["em"], result["AvNA"], is_best)
    ...
    def _train_epoch(self, epoch):
            ...
            if self.step % self.print_freq == self.print_freq - 1:
                ...
                print(("step: {}/{} \t "
                       ...
                       "loss: {}").format(
                           ...
                           batch_loss))
                self.tbx.add_scalar('train/NLL', batch_loss, self.step)
                global_loss = 0.0
                last_step = self.step
                last_time = time.time()
            self.step += 1
        ...
        metrics = self._valid_epoch(self.dev_eval_dict, self.dev_data_loader)
        print("dev_em: %f \t dev_f1: %f \t dev_avna: %f" % (
              metrics["exact_match"], metrics["f1"], metrics["AvNA"]))
              
        self.tbx.add_scalar(f'dev/EM', metrics["exact_match"], self.step)
        self.tbx.add_scalar(f'dev/F1', metrics["f1"], self.step)
        self.tbx.add_scalar(f'dev/AvNA', metrics["AvNA"], self.step)
        
        viz(self.tbx, pred_dict=self.dev_eval_dict, eval_path=self.dev_file,
            step=self.step, split='dev', num_visuals=10)

        result = {}
        result["em"] = metrics["exact_match"]
        result["f1"] = metrics["f1"]
        result["AvNA"] = metrics["AvNA"]
        return result

    def _valid_epoch(self, eval_dict, data_loader):
        """
        Evaluate model over development dataset.
        Return the metrics: em, f1, AvNA.
        """
        if self.use_ema and self.ema is not None:
    ...
    def _save_checkpoint(self, epoch, f1, em, avna, is_best):
        if self.use_ema and self.ema is not None:
            self.ema.assign(self.model)
        ...
        filename = os.path.join(
            self.save_dir,
            self.identifier +
            'checkpoint_epoch{:02d}_f1_{:.5f}_em_{:.5f}_avna_{:}.pth.tar'.format(
                epoch, f1, em, avna))
